...
#### 风险提示
- **这是本轮唯一的"请求体从未被真实验证"的工具**。若步骤 69 的 F5/F6 表明聚合端点不可用（400/500/超时），本步骤**必须整体回退**：本轮范围缩减为 8 项、工具总数按 **28** 计（若同时未通过步骤 75 的击杀条件，则为 **27**），并在 buildlog 与范围小节中如实标注"聚合维度因上游不可用顺延"。**严禁**用猜测的请求体硬上线，或把失败包装成"偶发"。
...
1. 先读步骤 69 的 F17 结论，据此二选一：
   - **三种组合全部 200** → 按下方代码注册该工具，本版本工具总数按 **29** 计；
   - **任意组合非 200** → **不注册该工具**，在 `buildlog.md` 中记录失败证据（状态码 + 响应体摘要 + 请求形态），工具总数按 **28** 计；步骤 76 的文档同步与步骤 77 的计数一律按 28 执行。
2. 注册实现（仅在通过击杀条件时执行）：
```python
@server.tool()
...
5. 本步骤结束时 `list_tools()` 为 **29 个**（通过）或 **28 个**（未通过）；两者都是本步骤的合法终态，取决于判定而非执行质量。

#### 验证方法
- `list_tools()` 工具数与本步骤第 1 条的判定一致（29 或 28），且 `core_output_search_by_query` 的出现/缺席与判定一致。
- 通过时的真实调用：
...
- 若未通过击杀条件，README 中"29/28"的计数与 CORE 工具表必须同步为"不含 outputs 关键词检索"，并在表格中**不出现**该行（不要留空行或注释行）。
...
#### 目标说明
CORE 工具数由 1 变为 8（或含条件项 9），工具总数由 21 变为 **28（或含条件项 29）**——注意现有 21 个工具里**已经包含** CORE 的 1 个工具，因此总数是"21 + 本轮新增工具数（7 或 8）"，不是"CORE 扩展后的工具数单独叠加"；同时 QA-R021 明确要求更正仓库内关于 CORE 限流与文档入口的失实表述。本步骤把代码变更与文档、回归脚本一次性对齐，避免留下"文档写了但代码没有"或"代码变了但文档没跟"的任何一侧不一致——这是本项目历次版本（v2.2.0 步骤 17、v2.3.0 步骤 23、v3.3.0 步骤 55）都特别点名过的高频事故点。

**计数一律以 `list_tools()` 返回值为准**，不得用 `rg -c "@server.tool"` 直接计数（历史上有注释中的该字符串导致虚高的先例，`goal.md` QA-R018 已明确禁止）。

#### 具体操作
1. **先确定本版本的实际工具数**：执行 `create_server()` → `list_tools()`，记下真实数量（预期 **28** 或 **29**）；后续所有计数改写以该值为准，**不得**在各文档里分别推算。
2. `README.md`（中文上位）与 `README_EN.md`（英文）逐处同步，**两份必须逐项对齐**：
   - 首段"**9 个数据源、21 个工具**" → "9 个数据源、**28/29 个工具**"（按实际值）；
   - §数据源总览后的"共提供 **21 个工具**"；
...
- `AGENTS.md` 中的工具计数**只在项目概述段落出现一次**（`**9 data sources / 21 tools**`，实测 `rg -n "\b21\b" AGENTS.md` 仅 1 行命中），但 `AGENTS.md` 的数据源范围表里 CORE 行另有 `max_results ≤ 25` 的旧上限（步骤 71 已改为 100）与"约 5 次请求后约 10 分钟锁死"的失实限流口径——**这两处不同性质的地方都必须在同一步里改掉**，否则会形成"计数对了但表格还写着旧上限"的新不一致。
...
2. `project-docs/buildlog.md` 追加本轮记录，按既有格式（标题含日期与步骤号）逐条覆盖：
   ...
   - 步骤 77：版本号、工具总数、回归结果；
   - 结论区显式写明**工具总数（28 或 29）、CORE 单源工具数（8 或 9）、未发布状态**。
...
```
   第 1 条须输出 28 或 29，且 `core_` 前缀工具集合恰为本文档列出的 8 或 9 个；
...
- 若步骤 75 判定不纳入第 9 项，则 `buildlog.md`、README、`AGENTS.md`、回归脚本四处都必须按 **28** 口径一致，任何一处残留 29 都会形成新的不一致。
