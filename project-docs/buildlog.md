...
- **`teach.md` 的历史清除已执行（用户二次确认后）**：使用 `git filter-repo --path project-docs/teach.md --invert-paths` 重写全仓历史（解析 159 个提交），`HEAD` 由 `c400888` 变为 `50c82d8`，提交数 157 → **156**（其中 1 个"仅新增 teach.md"的提交因改写后变空而被自动裁剪，其余 5 个提交仅失去该文件、内容不变）。重写前已在仓库外（`%TEMP%\UniArticles_pre_purge_20260919.bundle`，2.9 MB）生成全量 bundle 作为回滚备份，该备份**位于仓库外、未入库、未推送**。随后重新挂载 `origin` 并以 `git push --force-with-lease` 强推覆盖 `main`（`+ c400888...50c82d8 (forced update)`）。
  - **发布后核对**：`git ls-remote origin main` = `50c82d8`；GitHub API 查询该路径的提交历史返回 **0 条**；任意 ref 的历史与对象库中均不再含 `project-docs/teach.md`；改写后代码完整性复测 `__version__` = `3.5.0`、`list_tools()` = 29、`core_` = 9，与改写前一致。
  - **两处不可控的残留（须如实记录）**：① GitHub 上存在 **2 个 fork**，fork 各自保留旧历史，上游强推无法影响它们；② 旧提交对象在被 GitHub 内部 GC 之前仍可能通过**精确 SHA 直连**访问（实测旧 `HEAD` `c400888` 的 API 端点仍返回 200），仓库所有者无法主动触发其回收，如需彻底清除需联系 GitHub Support。以上两点不代表本次重写未生效——上游 `main` 的可浏览历史确已不含该文件。

### 下一步计划
- ⏳ 待用户决定：历史中另有一个**同名易混但不同**的文件 `TEACHING.md`（根目录级，v0.2～v1.0.1 时期的开发日志，已在 `8a6721f` 从工作区删除，但仍留在历史中；内容包含本机绝对路径与对私有同级仓库 `UniArticles_Skills` 的引用）。本次按用户指令仅清除了 `project-docs/teach.md`，**未**顺手清除 `TEACHING.md`——是否一并重写清除待用户确认。
- ✅ `teach.md` 的历史清除已随本次重写 + 强推完成，不再有该项待办。
