...
22.（v3.0.0，QA-R013，已确认）dblp 最终定案：经用户三轮不同网络环境实测（① project-builder-cn 原始探测环境 TLS 握手失败 HTTP 000 → ② 用户第一次实测环境 TLS 成功但 HTTP 500 → ③ 用户第二次实测环境 DNS/TCP/TLS/HTTP 全部成功并返回真实 JSON 数据），确认 dblp 服务端本身可用、无需 API key，此前的失败是探测环境/网络路径波动所致，非服务下线，也非代码错误。**dblp 正式从"悬而未决"转入"确认落地"**，纳入 v3.0.0 通用检索型新数据源范围。**同时记录一条已知风险**：dblp 在实际使用中可能因网络环境波动出现间歇性连接失败（历史观测含 TLS 握手失败与一次性 HTTP 500），不代表服务下线或代码错误，建议在该数据源的工具实现和/或文档中向用户说明这一特性；具体处理方式（例如错误信息中的提示文案）留给 project-planner-cn/project-builder-cn 在实现阶段决定。至此 **v3.0.0 立项之初规划的 13 个数据源/功能点范围全部定案完毕，没有遗留的待用户决策事项**：12 个确认落地（OpenAlex、Crossref、Europe PMC、DOAJ、Zenodo、HAL、OpenAIRE、bioRxiv/medRxiv、ChEMBL、Semantic Scholar、CORE、dblp）+ 1 个明确排除（PMC）+ 0 个悬而未决，加上已单独确认的 arXiv 补 `doi` 字段。
23.（v3.1.0，QA-R014/QA-R015，已确认）重写后的 PubMed 检索工具（原 `pubmed_paper_search_by_query`，新文件 `pubmed.py`）改为直接调用 NCBI `esearch.fcgi`+`efetch.fcgi`，对真实关键词查询返回 200 并正确解析出文献列表；返回字段集合以 project-builder-cn 真实探测 EFetch XML 结构为准，**不强制**与现有 `paperscraper` 输出字段一一对齐，允许有增有减，但需如实反映数据源真实返回内容，不得凭空编造字段。
24.（v3.1.0，QA-R014/QA-R015，已确认）新增的 3 个独立工具——基于 `esummary.fcgi` 的批量轻量元数据查询、基于 `elink.fcgi`（`cmd=neighbor`）的相关文献查询、基于 `elink.fcgi`（`dbfrom=pubmed&db=pmc`）的 PMC 全文/引用关联查询——均能对真实 PMID 返回 200 并正确解析出对应字段/关联列表；命名遵循项目既定"数据源_对象_动作(_by_限定词)"风格，最终名称由 project-planner-cn 在构建计划书中确定。
...
- **（v3.0.0，QA-R010，部分已确认）新增数据源，均遵循现有 source-module 模式，在 `src/uniarticles/sources/` 下各建立独立脚本文件**，依据是调研本地参考项目 `reference-projects/paper-search-mcp-main/`、`reference-projects/research-superpower-main/` 后的核实结论（详见附录"实测可行性探测"章节新增小节）+ 用户在 QA-R010 的确认：
  ...
  4. **PMC、Europe PMC、DOAJ、CORE、Zenodo、HAL、dblp、OpenAIRE**（QA-R011 已确认全部纳入探测环节）：这 8 个数据源在 QA-R010 中被评估为"中等价值"，此前只核实了 README 描述与代码逻辑、未做过真实 API 探测；用户在 QA-R011 明确要求"把 11 个候选源全部纳入"且"对每一个源都进行真实性探测"、"先测试再说"（不预先取舍），因此这 8 个与前 3 个推荐重点候选（Semantic Scholar/OpenAlex/Crossref）合计 11 个，以同等地位纳入探测环节。**探测完成后的最终去留结论见 QA-R012（核心目标第 21 条）与 QA-R013（核心目标第 22 条）**：Europe PMC、DOAJ、Zenodo、HAL、OpenAIRE 连同 Semantic Scholar/OpenAlex/Crossref 共 9 项已由 project-builder-cn 步骤 28-33 直接实测落地；CORE 因用户已配置 `CORE_API_KEY` 一并确认落地；PMC 已排除（见范围界定/排除）；**dblp 经用户三轮不同网络环境实测后于 QA-R013 确认落地**（历史观测含 TLS 握手失败、HTTP 500，但也有完全成功并返回真实数据的情况，已记录为已知风险，见约束条件）。至此 11 个通用检索型候选全部定案：10 个确认落地（含 dblp）、1 个排除（PMC），0 个悬而未决。
- **（v3.0.0，QA-R010，已确认）arXiv 三个现有工具补充 `doi` 输出字段**，复用第三方 `arxiv` 库 `Result.doi` 属性，零额外请求成本，作为独立步骤，可先于新数据源接入完成。
- **（v3.0.0，QA-R010/QA-R011，已确认）v3.0.0 立项之初范围合计规划新增 13 个数据源/功能点，是本项目至今规模最大的一轮版本**：11 个通用检索型新数据源（Semantic Scholar、OpenAlex、Crossref、PMC、Europe PMC、DOAJ、CORE、Zenodo、HAL、dblp、OpenAIRE）+ 2 个语义特殊的新数据源（bioRxiv/medRxiv 的浏览语义、ChEMBL 的 DOI 查询语义）+ 1 个现有工具增强（arXiv 三工具补 `doi` 字段）。**经 project-builder-cn 步骤 28-33 真实探测、QA-R012 与 QA-R013 用户最终裁决后（详见核心目标第 21、22 条），13 个立项候选中已确认落地 12 个**（OpenAlex、Crossref、Europe PMC、DOAJ、Zenodo、HAL、OpenAIRE、bioRxiv/medRxiv、ChEMBL 共 9 个经真实探测直接落地，Semantic Scholar、CORE 经 QA-R012 用户裁决补充确认落地，**dblp 经用户三轮网络环境实测后于 QA-R013 补充确认落地**）+ **arXiv 补 `doi` 字段独立落地**，**明确排除 1 个（PMC）**，**0 个悬而未决**。至此 v3.0.0 立项之初规划的 13 个数据源/功能点范围已全部定案完毕，没有遗留的待用户决策事项。project-planner-cn/project-builder-cn 制定后续文档时应引用 QA-R013 的最新结论作为范围现状，不再引用立项之初的"13 个"或 QA-R012 阶段"11 个确认落地"作为最终交付数量的等价表述。
- **（v3.1.0，QA-R014/QA-R015，已确认）PubMed 数据源从第三方包间接封装改为直连 NCBI Entrez API，并新增 3 项能力**，是 v3.0.0 发布并全部定案（QA-R013）之后的新一轮独立目标，源自用户重启此前作废的 QA-R009（v2.4.0 调研）遗留决策点：
  1. **重写 `pubmed_paper_search_by_query`**（对应新文件 `src/uniarticles/sources/pubmed.py`）：底层由"调用第三方包 `paperscraper`"改为"直接 `httpx` 调用 `esearch.fcgi`（检索 PMID 列表）+ `efetch.fcgi`（按 PMID 批量拉取 XML 并解析）"，移除对第三方包 `paperscraper`/`pymed-paperscraper` 的依赖。
  2. **新增 ESummary 批量元数据工具**：接入 `esummary.fcgi`，比 EFetch 更轻量，额外含 PMCID/PII/期刊全名/发表状态历史等现有实现没有的字段。
  3. **新增 ELink 相关文献查询工具**：接入 `elink.fcgi`（`cmd=neighbor`），给定 PMID 返回主题相关的其他 PubMed 文献 PMID 列表。
  4. **新增 ELink PMC 全文/引用关联查询工具**：接入 `elink.fcgi`（`dbfrom=pubmed&db=pmc`），给定 PMID 返回该文献在 PMC 是否有开放获取全文、以及哪些 PMC 文章引用/关联了它。
  5. **新增可选 `NCBI_API_KEY` 环境变量支持**，无条件注册模式（对齐 Elsevier/CORE 先例，非 Semantic Scholar 式条件注册），配置后可将 NCBI 官方限速从 3 请求/秒提升到 10 请求/秒。
  6. **工具总数由当前 25 个（或配置 `SEMANTIC_SCHOLAR_API_KEY` 时 27 个）增至 28 个（或 30 个）**。
  7. **命名、字段归一化细节、EFetch/ESummary/ELink 响应结构的确认，均需 project-builder-cn 先做真实 API 探测后再确定，不得凭空编写**——QA-R009（已作废但探测结论可复用，见 QA-R014 提问目的）曾用真实请求确认三项候选端点均返回 200 且不需要 Key，但探测时间已过去数日，仍建议按本项目"真实验证优先"惯例在正式实现前重新核实一次响应结构与限速表现是否有变化，不应仅凭历史探测结果直接下结论。
...
- **（v3.1.0，QA-R014/QA-R015，已确认）`NCBI_API_KEY` 注册模式明确对齐 Elsevier/CORE 的"无条件注册"先例，而非 Semantic Scholar 的"按 key 条件注册"先例**：这是继 QA-R012 引入 Semantic Scholar"无 key 则不注册工具"这一新架构分支后，本项目第二次面对"是否要条件注册"的选择，用户本轮明确选择维持默认的无条件注册模式，不扩大条件注册模式的适用范围。依据：NCBI 官方 Entrez API 无 key 也完全可用（仅限速从 10 请求/秒降到 3 请求/秒），缺 key 不会导致确定性失败，与 Semantic Scholar"无 key 必 429"的情况本质不同——project-planner-cn 在构建计划书中应清晰说明这一区分依据，避免不必要地把条件注册模式泛化到所有需要 key 的数据源。
- **（v3.1.0，QA-R014/QA-R015，已确认）本轮是一次面向已发布工具的破坏性变更（源码文件重命名 + JSON 响应体 `source` 字段值变化），用户已明确接受这一行为变化**：不需要设计过渡期、兼容层或新旧 `source` 字段值并存，比照 v2.2.0（QA-R004）"重命名无过渡期"先例处理即可，不需要为此单独设计迁移方案。
- **（v3.1.0，QA-R014/QA-R015，已确认，字段方案不预先规定）重写后检索工具的返回字段集合不强制与现有 `paperscraper` 输出对齐**：允许 project-builder-cn 按真实 EFetch XML 探测结果如实确定字段，可能相对现有实现有增有减；这与本项目此前处理 `get_abstract_details`/`retrieve_article`（QA-R006）、`serial_title_search`/`subject_classifications`（QA-R007/QA-R008）归一化任务时"先探测再定字段，不凭空编字段"的一贯做法一致。
- **（v3.1.0，QA-R014，探测结论复用说明）本轮大量决策依据已作废的 QA-R009（2026-08-04，v2.4.0 调研）真实探测结论**（`paperscraper` 依赖臃肿、`pymed_paperscraper` 不支持 `api_key` 参数、ESummary/ELink 三项候选端点实测 200 可用且无需 Key），复用这些结论是为避免重复劳动，但探测本身发生在数日之前，project-builder-cn 在正式实现前仍应按本项目"真实验证优先"惯例重新核实一次响应结构、字段命名与限速表现是否有变化，不应仅凭历史探测记录直接编写实现代码。
...
| Outputs 关键词检索 | `GET /v3/search/outputs/?q=` | **结果不一致，需复测**：creator 环境两次 500（上游 Azure Search 报 `Invalid expression: The operand for a binary operator 'Equal' is not a single value`）；/root 环境**四种组合 4/4 全部 200**——`q=machine learning&limit=2` → 81,738 B / totalHits 9,236,908；`q=title:"machine learning"` → 3,396 B；`q=doi:"10.1007/s10994-024-06619-7"` → 3,468 B / **totalHits 1（精确命中）**；`limit=100` → **1,145,330 B**。按 QA-R013 口径标注为"疑似与查询表达式相关或上游间歇性故障"，**不得定性为失败**。附带事实：响应体极大（outputs 记录默认内联 `fullText`）；`q=doi:"..."` 可精确命中单条 output，该记录含 `downloadUrl` / `sourceFulltextUrls` / `license` / `fulltextStatus` / `repositories` / `sdg` | ⚠️ 未接入，需复测 |
...
| Works 按 **裸 DOI** 取详情（/root 补充实测） | `GET /v3/works/{doi}` | **200**：`GET /v3/works/10.1038/nature12373` 返回该 DOI 的正确记录（作者 P. K. Lo / M. D. Lukin 等）。但 `GET /v3/works/doi:10.1038/nature12373`（带 `doi:` 前缀）→ **404 No route found**，即**前缀写法不通、裸 DOI 通**（与 journals 端点恰好相反）。边界：DOI 路线**只支持主资源**——`/works/{doi}/stats` → 200（返回 `{"id":72404683,...}`，可见内部能解析到 CORE id），但 `/works/{doi}/outputs` → **404**；未知 DOI → 404 且响应体为空 message `{"message":""}`（错误不可读，工具层需兜底文案） | ❌ 未接入（**works 详情类工具应同时接受 DOI 与 CORE ID 两种输入**，只收数字 CORE ID 对 LLM 调用方几乎不可用） |
| Work → 机构库画像链路（/root 补充实测） | `GET /v3/works/{id}` 的 `dataProviders` / `outputs` / `identifiers` 字段 → `GET /v3/data-providers/{id}` | **端到端 200 通过**：`/v3/works/171513974` 的 `dataProviders` 为 `[{"id":1630,"name":"Intellectum (Universidad de La Sabana)","url":".../v3/data-providers/1630"}]`，`outputs` 为 `["https://api.core.ac.uk/v3/outputs/641684400"]`，`identifiers` 同时含 `core_id` / `doi` / `oai_id` / `oai` 四种；`/v3/data-providers/1630` → 200 | ❌ 未接入（"机构库画像"候选的可行性依据） |
| Journals 按 ISSN 或 CORE ID 查询 / 统计 | `GET /v3/journals/issn:{issn}`、`/v3/journals/{id}`、`/{id}/stats` | **200 但返回空壳记录，无可用信息**：`/v3/journals/issn:0028-0836` → 200 / 129 B，`{"identifiers":["issn:0028-0836"],"publisher":null,"subjects":["Multidisciplinary"],"dataProviderId":"","title":"issn:0028-0836"}`（`title` 只是把输入标识符原样回显、`publisher` 为 null、`dataProviderId` 为空串，仅 `subjects` 真实）；`/v3/journals/0092-8674`（裸 ISSN）与 `/v3/journals/1`（纯数字 ID）均 **404 No route found**；`/v3/journals/issn:0028-0836/stats` → 200 / 68 B `{"identifier":1098,"countMetadata":0,"countFulltext":0,"doiCount":0}`（能拿到真实 CORE journal id，但三个计数全为 0） | ❌ 未接入，**且建议不纳入**：端点 200 但不提供刊名 / 出版商 / OA 状态等既有产品语义所需的任何信息 |
...
3. **但新增维度确实不与现有 9 个源重叠**：Data Providers（机构库画像）与聚合统计（分布/facet）在现有工具集中没有对应物，属于**新查询维度**，而不是"又一个关键词检索源"。**Journals（期刊维度）经 /root 补充实测已确认无价值**（端点虽返回 200，但只给回显式空壳记录，见上表），不再作为候选维度。
4. **按 QA-R013 流程必须由 project-builder-cn 复测的项**：journals 关键词检索连续超时、discover / recommend 各 500、TEI 返回 0 字节，以及 **outputs 检索在 creator / root 两个环境结果不一致（500 vs 4/4 全 200）**。本 agent 不得据本机单次结果把这些能力定性为"不可用"；复测脚本应写入 `_verify/`（creator 角色本轮只允许写 `project-docs/goal.md`，不代写该目录）。
...
6. **现有 `core_work_search_by_query` 存在已实测的性能浪费（属修既有缺陷，非新增能力）**：works 检索响应**默认内联 `fullText`**，实测 `GET /v3/search/works?q=machine learning&limit=25`（正是现有工具的 `max_results` 上限）→ **676,692 字节 / 4.8 秒**，其中 `fullText` 占 **536,574 字符**，而现有 `_normalize()` 拿到之后又把它整个丢弃；改用 `POST /v3/search/works` + `exclude:["fullText"]` 后同样的 25 条 → **112,549 字节 / 2.8 秒**（体积降至约 1/6、耗时降至约 6 折）。这是"现有工具增强"档里性价比最高的一项。
7. **限流口径三处不一致（应在实现阶段一并更正）**：官方文档现行口径为 token 制（未认证 100 tokens/天、10 次/分钟且**不提供 fullText**；注册个人 1,000/天、25 次/分钟）；`https://core.ac.uk/services/api` 页面另写"每 10 秒 1 个批量请求或 5 个单次请求"；而仓库现有 `src/uniarticles/sources/core.py` 的注释与 429 文案、以及 `AGENTS.md` 写的是"无 key 约 5 次请求后约 10 分钟锁死"。本机实测响应头为 `x-ratelimit-limit: 150` / `x-ratelimit-remaining: N`（普通查询逐次约扣 1）/ `x-ratelimit-retry-after: <ISO 时间戳>`（认证态），匿名态 429 时为 `limit=10`。三者不同源，**现有仓库表述需更正**。
...
### QA-R013：dblp 三轮网络环境实测结果汇总定案 + 新增"失败验证脚本必须输出到 _verify/ 供用户独立验证"通用流程约束
<!-- GOAL-QA-R013-START -->
- **提问时间**：2026-08-05 20:53
- **提问目的**：QA-R012 将 dblp 标记为"待定"，原因是 project-builder-cn 步骤 28-33 的探测环境对 dblp.org 的 TLS 握手直接失败（HTTP 000），requests/curl 双栈一致，无法判断是 dblp 服务本身不可用还是探测环境网络受限。project-builder-cn 随后在 `_verify/dblp_connectivity_test.py` 建立了 DNS→TCP→TLS→HTTP 四层分层诊断脚本（commit `9948687`，后续增强脱敏与错误响应体打印，commit `abd3e3f`），交由用户在不同网络环境下反复实测，现汇总用户三次实测结果并请用户给出最终定案；同时用户在交流中主动提出一条面向 v3.0.0 剩余全部实现工作的新流程要求，需一并记录为正式约束。
- **问题列表**
  1. dblp 三次网络环境实测结果依次为：①（project-builder-cn 原始探测环境，buildlog commit `008d509`）TLS 握手直接失败，HTTP 状态码 000；②（用户第一次实测环境）DNS/TCP/TLS 全部成功（TLSv1.3，证书正确），但 HTTP 请求返回 500（服务端错误）；③（用户第二次实测环境，脱敏增强版脚本）DNS/TCP/TLS/HTTP 全部成功，HTTP 200，返回真实 JSON 数据（`{"result":{"query":"graph*","status":{"@code":"200","text":"OK"},...}}`，字段结构与 dblp 官方 search API 文档一致）。三次结果分别卡在不同层，dblp 公开检索 API 本身无 key 机制，实测中从未出现 401/403，说明失败与鉴权无关。综合这三次结果，dblp 的最终去留应如何定案？
  2. 本次 dblp 反复实测暴露出一个问题：project-builder-cn 自身探测环境的失败结果（TLS 握手失败）不能直接等同于目标服务不可用，且用户在不同网络环境下两次复测又给出了不同的中间结果（HTTP 500、HTTP 200）。今后 v3.0.0 剩余数据源的构建/测试工作中，如果 agent 在自己的探测环境中遇到失败情况（尤其是网络类失败），应该如何处理，才能避免仅凭 agent 自身单次探测结果就下结论？
- **用户回答**
  1. "以上为输出结果，返回可信，代表 dblp 相关的内容可以继续开发，但可能需要标注强调因网络环境波动而出现失败的情况。"
  2. "接下来在构建脚本时，如果在测试中遇到失败的情况需要将验证脚本输出到 _verify 下后让我单独验证。"
- **提炼结论**
  - **问题1（已确认，dblp 最终定案为纳入）**：dblp 正式从"悬而未决"转入"确认落地"，纳入 v3.0.0 通用检索型新数据源范围。三次实测（TLS 层失败 → 应用层 500 → 完全成功且返回真实数据）说明 dblp 服务端本身可用、能返回真实数据，此前失败是探测环境/网络路径波动所致，不是 dblp 服务下线，也不是需要 API key（dblp 公开检索 API 本身无 key 机制）。**用户同时明确要求标注一条风险提示**：dblp 在实际使用中可能因网络环境波动出现间歇性连接失败（历史观测：TLS 握手失败、HTTP 500 均出现过，也有完全成功的情况），这不代表服务下线或代码错误，建议在该数据源的工具实现和/或文档中向用户说明这一特性；具体处理方式（例如是否在错误信息中加特别提示文案）留给 project-planner-cn/project-builder-cn 在后续实现阶段决定，本文档只记录这个已知特性本身。
  - **问题2（已确认，新增通用流程约束，非本次临时安排）**：用户明确提出一条标准工作流程规则，适用范围是**今后（v3.0.0 剩余数据源实现阶段及以后）任何构建/测试环节**，不局限于 dblp 本身——一旦 agent（project-builder-cn 等）在自己的探测/验证环境中遇到失败结果，不得仅凭自身环境的单次失败结果下结论（尤其是网络类失败），必须把对应的验证脚本产出到仓库 `_verify/` 目录下，交由用户在其独立网络环境下亲自验证，作为最终判断依据之一，而不是仅依赖 agent 自身探测环境的单次结果。这是本次 dblp 三轮反复实测（agent 环境失败、用户环境两次结果不同）得出的经验教训，用户明确希望这个做法成为今后的标准流程，已记入"约束条件"章节。
  - **范围现状收尾**：至此 v3.0.0 立项之初规划的 13 个数据源/功能点全部定案完毕，不再有遗留的待用户决策事项——12 个确认落地（OpenAlex、Crossref、Europe PMC、DOAJ、Zenodo、HAL、OpenAIRE、bioRxiv/medRxiv、ChEMBL、Semantic Scholar、CORE、dblp）+ 1 个明确排除（PMC）+ 0 个悬而未决，加上已单独确认的 arXiv 补 `doi` 字段。可正式移交 `project-planner-cn` 续写 `project-plan.md` 步骤 34+。
- **影响的目标文档章节**
  - 核心目标（新增第 22 条） / 范围界定（包含/排除现状更新） / 约束条件（dblp 风险提示定案 + 新增通用验证脚本流程约束） / 备注（正式移交 project-planner-cn）
  <!-- GOAL-QA-R013-END -->

### QA-R014：v3.0.0 之后新一轮调研——PubMed 检索改为直连 NCBI Entrez API（重启 QA-R009 未决决策点）
<!-- GOAL-QA-R014-START -->
- **提问时间**：2026-08-06 23:40
- **提问目的**：v3.0.0 已在 QA-R013 全部定案并移交 project-planner-cn。用户现提出新想法："我希望直接通过 NCBI 端点来查询 PubMed"，即放弃 `src/uniarticles/sources/paperscraper.py` 中对第三方包 `paperscraper`（内部依赖 `pymed_paperscraper`）的间接封装，改为像 `openalex.py`/`crossref.py`/`europepmc.py` 一样，直接用 `httpx` 调用 NCBI 官方 Entrez E-utilities（`esearch`/`efetch`/`esummary` 等，`https://eutils.ncbi.nlm.nih.gov/entrez/eutils/`）。

  这与此前 **QA-R009**（2026-08-04，v2.4.0 调研，后因用户主动跳过 v2.4.0 直接进入 v3.0.0 而被 `git revert c316c98` 移出文档、未获用户答复）几乎是同一个决策点的重启。QA-R009 当时已完成真实调研与真实 API 探测，结论可直接复用，避免重复劳动：
  1. **现状实现方式核实**：`paperscraper.py` 第 4 行调用第三方包 `paperscraper`（PyPI 0.3.6）的 `get_pubmed_papers()`，内部实例化其依赖 `pymed_paperscraper.PubMed`，依次调用 NCBI 官方 `esearch.fcgi`（检索 PMID 列表）与 `efetch.fcgi`（按 PMID 批量拉取 XML 并解析出 `title`/`authors`/`abstract`/`journal`/`doi`/`publication_date`/`keywords`/`methods`/`conclusions`/`results`/`copyrights` 等字段）。**不是网页爬虫，是对 NCBI 官方 API 的间接包装**，当前实现不支持也不使用 NCBI API Key。ESearch 官方限制单次最多取回前 9999 条，现有 `max_results` 已 clamp 到 `[1, 9998]`。
  2. **依赖臃肿问题（已用真实文件核实）**：第三方 `paperscraper` 包为了一个函数背负庞大依赖树——`scholarly`（Google Scholar 爬虫，v2.1.0 已删除 Google Scholar 工具但此依赖仍被静默安装、完全未使用）、`boto3`（AWS SDK）、`matplotlib`/`seaborn`/`matplotlib-venn`（绘图库）、`pytest`（测试框架被声明为运行时依赖）等。且该包在 `__init__.py` 顶层执行 `logging.basicConfig(stream=sys.stdout, ...)`，是 `src/uniarticles/__init__.py` 中那段 stderr-handler-抢占 workaround 注释的根因——只要仍依赖 `paperscraper`，隐患是被压制而非根除。`pymed_paperscraper` 这一层完全不支持传入 `api_key` 参数，即便配置了 NCBI Key 也无法在当前依赖链下生效换取提速。
  3. **已用真实 Entrez API 请求验证可行、且均不需要 Key 的候选新能力**：`esummary.fcgi`（200，比 EFetch 更轻量，额外含 PMCID/PII/期刊全名/发表状态历史等 EFetch 现有实现没有的字段）、`elink.fcgi`（`cmd=neighbor`，200，返回主题相关文献 PMID 列表）、`elink.fcgi`（`dbfrom=pubmed&db=pmc`，200，返回该文献在 PMC 是否有全文及引用/关联的 PMC 文章列表）。三者均不在 `paperscraper`/`pymed_paperscraper` 现有暴露接口范围内，无论是否整体替换依赖，只要想要这三项能力，就必须绕开 `paperscraper`、直接 httpx 调用。
  4. **命名连带影响**：`paperscraper` 同时是①本项目源码文件名、②`_ok`/`_err` 硬编码写入 JSON 响应体的 `"source"` 字段值（面向调用方可见）、③被包装的第三方 PyPI 依赖包名——三者恰好同名但相互独立，是当前命名混乱的根源。若改名为 `pubmed.py`，需同步改 `src/uniarticles/sources/__init__.py` 的 import 路径与 `register_paperscraper_source` 函数名、`CLAUDE.md`/`README.md`/`README_ZH.md`/`project-docs/teach.md` 中的引用；是否连带把 `source` 字段值也改成 `"pubmed"`，是一次面向调用方可见的行为变化，需用户明确表态。
  5. **NCBI 官方限速政策（文档记载，非本次少量请求能证伪）**：不带 API Key 3 请求/秒，带免费申请的 API Key 10 请求/秒。当前 `.env`/`.env.example` 均未配置任何 NCBI 相关 Key。

  基于以上已有结论，本轮向用户提出以下决策问题，聚焦"新想法与 QA-R009 三个方案（A 保守改名不动实现 / B 渐进混合新旧并存 / C 彻底重构直连httpx）如何对应"以及若干 QA-R009 未覆盖的新维度（版本号、`NCBI_API_KEY` 注册模式、字段对等性要求）。
- **问题列表**
  1. 你这次说的"直接通过 NCBI 端点查询"，是指 QA-R009 里的**方案C（彻底重构）**——把现有 `pubmed_paper_search_by_query` 也一并改造为直接 httpx 调 ESearch/EFetch、连带移除 `paperscraper`/`pymed-paperscraper` 依赖——吗？还是有别的范围设想（比如只想要 ESummary/ELink 这类现有工具没有的新能力，现有检索工具本身可以不动）？
  2. 除了"改为直连"这个动作本身，是否也想借这次机会把 QA-R009 已实测确认可行的 ESummary（按 PMID 批量取更轻量的元数据，含 PMCID/PII 等新字段）、ELink 相关文献查询、ELink PMC 全文/引用关联查询，选一部分或全部一并纳入本轮范围？还是这轮只做"检索方式迁移"，新能力留到以后单独评估？
  3. 是否要新增支持可选的 `NCBI_API_KEY` 环境变量（免费申请，3→10 请求/秒提速）？如果要，倾向哪种注册模式——类似 Elsevier/CORE（无条件注册工具，缺 key 时仅仅是限速更严格，不影响能否调用），还是类似 Semantic Scholar（缺 key 时不注册工具）？（提示：NCBI 官方 API 本身无 key 也完全可用，只是限速更严，与 Semantic Scholar"无 key 必 429"的情况不同，供你参考但由你决定）
- **用户回答**
  1. 选**方案C（彻底重构）**——现有 `pubmed_paper_search_by_query` 检索工具也一并改为直连 NCBI httpx（esearch/efetch），并移除 `paperscraper`/`pymed-paperscraper` 依赖，顺带解决其 stdout 污染的根因问题。
  2. **一并做进这一轮**，即本轮同时完成检索方式迁移 + 三项已验证新能力（ESummary 批量元数据、ELink 相关文献查询、ELink PMC 全文/引用关联查询）。
  3. **新增支持**，采用无条件注册模式（类似 Elsevier/CORE：不管有没有 key 都注册工具，缺 key 只是限速更严，符合 NCBI 无 key 也完全可用的实际情况）。用户确认 key 已配置在 `.env` 的 `NCBI_API_KEY` 变量中。
- **提炼结论**
  - **问题1（已确认）**：采用方案C。`pubmed_paper_search_by_query` 的底层实现从"调用第三方包 `paperscraper`"改为"直接 httpx 调用 NCBI `esearch.fcgi`+`efetch.fcgi`"，`pyproject.toml`/`uv.lock` 中的 `paperscraper` 依赖声明整体移除。`src/uniarticles/__init__.py` 中为压制 `paperscraper` 包 `logging.basicConfig(stream=sys.stdout, ...)` 而写的 stderr-handler-抢占 workaround 注释，其存在的根因将随依赖移除而消失——是否同步简化/更新该注释、还是保留作为通用防御措施，留给 project-builder-cn 在实现阶段按代码实际情况判断，不在本文档中预先规定。
  - **问题2（已确认）**：ESummary（批量轻量元数据）、ELink 相关文献查询、ELink PMC 全文/引用关联查询三项候选新能力全部纳入本轮范围，与检索方式迁移同批交付，不拆分成独立的后续版本。
  - **问题3（已确认）**：新增 `NCBI_API_KEY` 可选环境变量支持，注册模式对齐 Elsevier/CORE（无条件注册，缺 key 时仅限速更严、不影响是否能调用），不采用 Semantic Scholar 式的条件注册。key 已实际配置在 `.env`，具备真实调用验证条件。
- **影响的目标文档章节**
  - 核心目标 / 范围界定（包含/排除） / 约束条件
<!-- GOAL-QA-R014-END -->

### QA-R015：命名/字段对等性/版本号——收尾前的最后几个操作层面决策点
<!-- GOAL-QA-R015-START -->
- **提问时间**：2026-08-07 01:09
- **提问目的**：QA-R014 已确认本轮采用方案C（彻底重构 + 三项新能力 + `NCBI_API_KEY` 无条件注册），范围和技术路线已经清晰。但 QA-R009 遗留的"命名连带影响"（文件名/注册函数名/`source` 字段值三者同名但独立）以及新能力落地后必然出现的工具数量/命名、字段是否对等、目标版本号，这几个操作层面的问题此前只是列出了选项，还没有请用户逐一拍板。这些问题会直接决定 project-planner-cn 写构建计划书时的具体条目，必须在移交前收敛。
- **问题列表**
  1. 【命名】既然确认了方案C 会彻底重写这个模块，是否同步把源码文件从 `paperscraper.py` 改名为 `pubmed.py`（同步改 `src/uniarticles/sources/__init__.py` 的 import 路径与 `register_paperscraper_source` → `register_pubmed_source`、`CLAUDE.md`/`README.md`/`README_ZH.md`/`project-docs/teach.md` 中的引用），并把 `_ok`/`_err` 硬编码写入 JSON 响应体的 `"source"` 字段值也从 `"paperscraper"` 改成 `"pubmed"`？后者是一次面向调用方可见的行为变化（已发布工具 `pubmed_paper_search_by_query` 返回内容会变化），需要你明确确认是否接受。
  2. 【新工具组织方式】ESummary/ELink 相关文献/ELink PMC 关联这三项新能力，是希望做成 3 个独立的新 MCP 工具（工具总数从当前 25/27 增加到 28/30），还是有别的组织方式（例如合并成更少的工具、用参数区分行为）？具体工具命名会交给 project-planner-cn 按项目既定"数据源_对象_动作(_by_限定词)"风格拟定，这里只需要你确认"独立工具 vs 合并"的方向。
  3. 【字段对等性】重写后的检索工具（对应原 `pubmed_paper_search_by_query`）如果改成自己解析 EFetch 返回的 XML，是否要求必须覆盖现有实现已有的全部字段（`title`/`authors`/`abstract`/`journal`/`doi`/`publication_date`/`keywords`/`methods`/`conclusions`/`results`/`copyrights`），不能减少；还是允许 project-builder-cn 基于真实 XML 探测结果确定实际可提取字段（可能有增有减，只要真实、如实反映数据源实际返回内容）？
  4. 【版本号】当前 `pyproject.toml` 版本为 `3.0.0`（v3.0.0 已发布）。本轮改动（PubMed 直连重构 + 3 项新能力 + NCBI_API_KEY 支持）属于新增/重构混合性质，目标发布版本号定为多少？
- **用户回答**
  1. **全部改**——`paperscraper.py` → `pubmed.py`，import 路径、注册函数名、README/CLAUDE.md/teach.md 引用，以及 JSON 返回体里的 `"source"` 字段值也从 `"paperscraper"` 同步改成 `"pubmed"`。
  2. **3 个独立新工具**（ESummary、ELink 相关文献、ELink PMC 全文关联各一个），工具总数从 25/27 增加到 28/30，具体命名交给 project-planner-cn 按项目既定风格确定。
  3. **允许如实确定**（用户采纳本 agent 推荐方案）——按 project-builder-cn 真实探测 NCBI EFetch 返回的字段来定，不强求与现有 paperscraper 输出字段一一对齐，可能有增有减。
  4. **3.1.0**。
- **提炼结论**
  - **问题1（已确认）**：文件/函数/文档引用全部改名为 `pubmed.py` 体系，且 JSON 响应体 `"source"` 字段值同步从 `"paperscraper"` 改为 `"pubmed"`——用户明确接受这一面向调用方可见的行为变化。
  - **问题2（已确认）**：ESummary、ELink 相关文献查询、ELink PMC 全文/引用关联三项能力落地为 3 个独立新 MCP 工具，不合并、不做成可选参数分支。工具总数预期从当前 25 个（或配置 `SEMANTIC_SCHOLAR_API_KEY` 时 27 个）增至 28（或 30）个。具体工具命名遵循项目既定"数据源_对象_动作(_by_限定词)"风格，最终名称由 project-planner-cn 在构建计划书中拟定。
  - **问题3（已确认）**：不强制字段对等，字段集合以 project-builder-cn 真实探测 NCBI EFetch 返回结构为准，如实反映数据源实际能提供的内容，允许相对现有 `paperscraper` 输出有增有减。
  - **问题4（已确认）**：目标发布版本号为 **`3.1.0`**（当前 `pyproject.toml` 为 `3.0.0`）。
  - **至此 QA-R014/QA-R015 两轮问题全部获得用户明确回答，本轮（PubMed 直连 NCBI 重构）目标澄清已闭环，无遗留待定事项。**
- **影响的目标文档章节**
...
### QA-R017：数据源范围收缩——可排除候选与判定标准
<!-- GOAL-QA-R017-START -->
- **提问时间**：2026-09-18 14:49
- **提问目的**：用户提出"当前文献源有些太多了，如果要排除掉一些文献源，可以排除哪些"。这是继 QA-R016（移除 ChEMBL/HAL，15→13）之后又一次范围收缩意向，但本次用户未指定具体源，只给出"太多"这一感受性描述，因此需要先确认判定标准（工具选择噪声/上下文开销、维护成本、key 与限流门槛、可靠性与覆盖冗余），再据标准确定排除对象。本轮提问前先对当前 13 个已注册数据源做了一次"可替代性"梳理，作为提问依据，不预设结论：
  - **元聚合/高度重叠层**：OpenAIRE（欧洲开放科学聚合器，与 CORE/DOAJ/Zenodo 类源收录重叠较大）、CORE（OA 聚合器，无 key 时约 5 次请求后触发 ~10 分钟 429 锁定）、DOAJ（仅覆盖经审核的 OA 期刊，与 Crossref/OpenAlex 高度重叠）、Zenodo（已被硬过滤为 `type=publication`，等于放弃该库最主要的数据集/软件内容，剩余出版物记录又与 OpenAlex/Crossref 重叠）——这四个是"去掉后覆盖率损失相对最小"的一层。
  - **同族重复**：Crossref 与 OpenAlex（DOI 元数据高度重叠，OpenAlex 额外提供引用数与 OA 状态）；Europe PMC 与 PubMed（Europe PMC 索引包含 PubMed，额外提供预印本与 OA/PMC 标记）。
  - **内容独有但窄域或带已知风险**：dblp（仅计算机学科，且历史观测到 TLS 失败 / HTTP 500 / 429 的间歇性不可达）、bioRxiv/medRxiv（上游无关键词检索，只能按日期窗口浏览，固定 30 条/次）、ScienceDirect（无检索工具，仅标识符查询，且依赖 Elsevier 权限）。
  - **需单独说明的一类**：Semantic Scholar 为条件注册——未配置 `SEMANTIC_SCHOLAR_API_KEY` 时它注册 0 个工具，故"移除"它在无 key 场景下不减少任何工具数，仅在配置了 key 的场景下减少 2 个。
- **问题列表**
  1. 本次"太多"的核心困扰是什么——是工具数量多导致 LLM 在客户端里选择工具时更容易选错/上下文开销大，还是维护与 key/限流成本，还是对部分源的质量与稳定性不满意，抑或只是覆盖冗余看着重复？（不同动机下应砍的源不同：选前者应优先砍"元聚合/同族重复"层，选后者应优先砍 CORE/dblp/OpenAIRE 这类有门槛或可靠性问题的源。）
  2. 你的主要检索场景集中在哪个学科与文献类型（例如计算机、生物医学、社会科学；期刊论文为主还是也需要预印本/OA 全文），有没有哪几个源是"必须保留"的？（这一条决定 dblp、bioRxiv/medRxiv、ScienceDirect 这类内容独有但窄域的源是否值得留。）
  3. 期望收缩到什么规模，以及是否接受"免费、无 key、无门槛但内容有重叠"的源继续保留（例如 OpenAlex/Crossref 这类零配置成本的源，留着只增加工具数、不增加使用门槛）？
- **用户回答**
  （2026-09-18，逐字记录）
  1. "主要是工具受到干扰，部分工具无法直接检索到特定文献"
  2. "与学科无关，只与工具本身的可连接性与有效性有关"
  3. "暂时不考虑"
- **判据收敛（据回答 2 修正本轮提问目的中的分档依据）**
  - 有效判据：①**工具干扰**——检索形态的重复度，即有多少个工具长得像"输入关键词、返回文献列表"；②**可连接性**——在当前网络环境下能否稳定接通；③**定向检索有效性**——给定一篇已知文献，工具能否直接检索到它。
  - **失效判据**：学科相关性、"窄域"。本轮提问目的中"仅覆盖计算机学科""内容独有但窄域"等表述据此作废，不得作为排除依据。
  - 回答 3 明确暂不考虑中文数据库方向，故本轮不提出"补充万方/CNKI/维普"一类替代方案。
- **本轮补充实测（2026-09-18，本机环境，取代提问时的静态推测；对全部 14 个源发起真实调用）**
  - 连通性全量：24 次调用覆盖 14 个源 → 23 成功，仅 `dblp` 失败（HTTP 429 + 连接被重置）。`biorxiv` 调用成功、固定返回 30 条。
  - 定向检索矩阵：取 3 篇具体文献（CS 预印本《Attention Is All You Need》、生物医学《Genome engineering using the CRISPR-Cas9 system》、Elsevier 期刊《Hallmarks of Cancer: The Next Generation》），对 11 个关键词检索型工具各取 top-3 判定是否命中：
    - 命中 3/3：`openalex`、`crossref`、`europepmc`、`openaire`
    - 命中 2/3、另 1 次 HTTP 500：`core`
    - 命中 1/3：`pubmed`、`zenodo`、`doaj`
    - 命中 0/3：`scopus`、`arxiv`
    - 可连接性 0/4：`dblp`
    - 结构性无法定向检索：`biorxiv`（上游无关键词检索）
  - **根因定位**——这是回答 1 后半句"部分工具无法直接检索到特定文献"的真实成因，分三类，不得混为一谈：
    1. **排序策略问题（可修复，不属于被删源的缺陷）**：`scopus.py` 默认 `sort="coverDate"`；`arxiv.py` 硬编码 `sort_by=SubmittedDate`；`pubmed.py` 未传 `sort` 参数，而 NCBI ESearch 原始 API 的默认序是日期序、并非网页端的 Best Match。三者都会让"用标题找论文"这一最常见诉求返回"命中若干关键词的最新文献"而非目标文献。对照实测：`scopus` 把排序改为 `relevancy` 后 3/3 命中，改用 `TITLE("...")` 字段查询后 top-1 精确命中；`arxiv` 改用 `ti:"..."` 后正确命中（另两篇本就不在 arXiv，返回 0 条属正确行为，不算失败）。
    2. **上游 API 结构性不支持（不可修复）**：`biorxiv.py` 只能按 `server` + 日期窗口 + cursor 浏览、固定 30 条/次，无法对一篇已知文献做定向检索。
    3. **可连接性问题**：`dblp` 本机 0/4（HTTP 429 / 连接被重置 / 非 JSON 响应），与 QA-R013 记录的间歇性不可达性质一致；按该条既定流程规则，此现象本身不构成"技术不可行"结论，本轮仅作为"可连接性"判据下的一条实测记录。
  - 附带发现（与删源无关的独立优化点）：26 个工具的名称+描述合计 7041 字符（约 2000 tokens）；其中 `dblp` 单个工具占 823 字符（11.7%），因其 tool description 内嵌了网络波动警告文案；`scopus`(26.6%) + `pubmed`(18.2%) + `dblp`(11.7%) + `sciencedirect`(9.6%) 四者合计占 66%。
- **提炼结论**
  - 判据已按用户回答收敛为"工具干扰 + 可连接性 + 定向检索有效性"。据此，本轮排除性质应以"**检索形态重复造成的工具干扰**"与"**定向检索有效性/可连接性不达标**"为主，并与既有三类排除性质严格区分：QA-R003（权限受限，401/超时）、QA-R012（技术不可行，PMC）、QA-R016（产品价值收窄，ChEMBL/HAL）。
  - **可独立于删源单独执行的修复**（不减少工具数、不属于任何排除方案）：`scopus_document_search_by_query` 默认排序改为相关性、`pubmed_paper_search_by_query` 显式传 `sort=relevance`、评估 `arxiv.py` 的硬编码 SubmittedDate 排序。这是回答 1 后半句的直接病根；若不修，删掉再多源也仍会出现"查不到那篇文献"的现象。
  - **方案 A（保守：只删有效性/可连接性不达标的源）**：删除 `biorxiv.py`（结构性无法定向检索，1 个工具）、`dblp.py`（本机 0/4 可连接，1 个工具）→ 源 14→12、工具 26→24；源码减少 204 行（biorxiv 91 + dblp 113，占源文件总量 2137 行的 9.5%）；工具描述减少 1130 字符（307 + 823，占 7041 的 16.0%）。
  - **方案 B（推荐，直指"工具干扰"这一主要困扰）**：在方案 A 之上再删除 4 个检索形态重复的源 `openaire.py`、`core.py`、`zenodo.py`、`doaj.py` → 源 14→8、工具 26→20（配置 `SEMANTIC_SCHOLAR_API_KEY` 时为 22）；源码减少 545 行（25.5%）；工具描述降至 4947 字符（-30%）；"关键词检索型"工具由 12 个降至 5 个（scopus / arxiv / pubmed / openalex / crossref，另有条件注册的 semantic_scholar），这是干扰量的直接下降。被删的 4 个源均满足"形态重复"且各有实测支撑：`openaire` 处于聚合链最上游、内容为其他源的再聚合；`core` 无 key 时约 5 次请求即锁定约 10 分钟、本轮另有 1 次 HTTP 500；`zenodo` 因 `type=publication` 硬过滤已放弃其数据集/软件这一主要差异化内容；`doaj` 对非 OA 期刊目标命中 1/3。
  - **保留集合**（方案 B 之后）：`scopus`（受控索引 + 引用数 + 期刊元数据 + 学科分类代码）、`sciencedirect`（全库唯一提供图表/补充材料清单；注意它没有任何检索工具，只能按标识符取回，定向检索由 Scopus 承接）、`arxiv`、`pubmed`、`openalex`、`crossref`、`europepmc`（定向检索 3/3 命中，且提供 PubMed 缺失的引用数与预印本覆盖）、（可选）`semantic_scholar`。
  - **能力不下降的论证**：定向检索能力由 `scopus`/`openalex`/`crossref`（跨学科）、`pubmed`/`europepmc`（生物医学）、`arxiv`（预印本）承接；被删 6 个源中，`openaire`/`core`/`zenodo`/`doaj` 的收录内容均为上述保留源的子集或再聚合，`biorxiv`/`dblp` 的删除依据是有效性/可连接性——均不构成"内容覆盖下降"。
  - 备选调整：若日后需要"只在经审核的 OA 期刊中检索"这一语义（该语义 Crossref/OpenAlex 结构上无法复制），可保留 `doaj.py`，方案 B 退化为删 5 个源、工具 26→21。
  - **用户决策（2026-09-18）**：采用**方案 A，并额外移除 Zenodo**——本轮删除 `biorxiv.py`、`dblp.py`、`zenodo.py` 三个源文件。最终范围：数据源 14→11（默认激活 13→10；Semantic Scholar 因条件注册不计入默认集合）、工具 26→23（配置 `SEMANTIC_SCHOLAR_API_KEY` 时 28→25）；源码减少 280 行（biorxiv 91 + dblp 113 + zenodo 76，占源文件总量 2137 行的 13.1%）；工具描述减少 1379 字符（307 + 823 + 249），总量 7041→5662（-19.6%）；"关键词检索型"工具由 12 个降至 10 个（减少 `dblp`、`zenodo` 两个；`biorxiv` 本就不属该形态）。
  - **本轮排除性质的归类（不得混写）**：`biorxiv` 属"上游 API 结构性不支持定向检索"（本集合中唯一一例）；`dblp` 属"可连接性不达标"（本机 0/4，性质与 QA-R013 记录的间歇性不可达一致——按该条流程规则，此记录不构成"技术不可行"结论）；`zenodo` 属"检索形态重复造成的工具干扰"（其差异化内容已被 `type=publication` 硬过滤放弃），与 QA-R016 的"产品价值收窄"性质相近但判据不同。三者均**不属于** QA-R003 的权限受限、QA-R012 的技术不可行。
  - **仍未确认事项**：① 三处排序修复（`scopus.py` 默认排序改为相关性、`pubmed.py` 显式传 `sort=relevance`、评估 `arxiv.py` 硬编码 SubmittedDate）是否纳入本轮——用户本轮未表态，**默认不纳入**，留待后续按 project-plan.md 增量追加步骤的方式补入；② 版本号（依既有惯例建议 `3.3.0`，比照 QA-R016 先例，需在构建计划书阶段由用户确认）。本轮构建计划书应按上述已确认范围拟制。
- **影响的目标文档章节**
...
### QA-R019：移除 OpenAlex 数据源（匿名检索端点持续性限流）
<!-- GOAL-QA-R019-START -->
- **提问时间**：2026-09-18
- **提问目的**：用户直接下达范围指令，无澄清问答："既然openalex经常出问题，那就把它也移除了，然后更新版本到3.4.0。"
- **用户回答**：无需提问，指令明确（"经常出问题"即移除原因）。
- **落地前的实测核实（2026-09-18，本机环境）**
  - 匿名关键词检索 `api.openalex.org/works?search=...`：本轮动手前复测仍返回 **HTTP 429**（响应体自述 `Anonymous search is temporarily rate-limited while the search cluster is under elevated load`，响应含 `retry-after: 30`）。
  - 同一天较早的 v3.4.0 回归验证中，该端点已持续 **1 小时以上**不可用，重试 3 次（间隔 30 秒）仍全部 429。
  - 同一时刻按 DOI 的详情端点 `openalex_work_detail_by_doi` 返回 **HTTP 200**——即"search 端点单独降级"，而非整站下线。
- **决策与判据**
  - 采用**整个数据源移除**（连带 2 个工具），而非"加退避自动重试"或"申请免费 API Key 后保留"：该源的 Key 属即时申请、无机构审批门槛（与 QA-R018 的 Semantic Scholar 情形不同），但用户在本轮明确选择直接移除；同时该源检索能力的可用性取决于上游搜索集群的匿名配额，属无法在代码层根治的外部状态。
  - 排除性质：**上游可用性/限流不稳定**。与 QA-R013（dblp，间歇性连接失败，判定服务端本身可用故保留）、QA-R018（Semantic Scholar，外部授权准入受限）性质均不同——本轮不是"拿不到凭证"，而是"不持凭证时的主力工具被上游持续性限流"，且用户不选择以凭证方式规避。
  - 与前几轮的共同点：**既不是技术不可行、也不是权限受限**——代码路径本身正确（DOI 详情端点始终正常），移除纯属对不稳定外部依赖的范围收缩。不得据此认为 OpenAlex 的接口实现有缺陷。
- **影响范围**
  - 源码：删除 `src/uniarticles/sources/openalex.py`（含倒排索引摘要重建逻辑）；清理 `sources/__init__.py` 中对应的 import 与 `register_openalex_source(server)` 注册调用。
  - 用户文档：`README.md`、`README_ZH.md`——特性列表中的"通用学术检索"源清单、数据源对比表格行、`### OpenAlex` 独立小节（含 2 个工具行与 429 风险说明）、工具可用性实测段落中的计数与结论、"文献查找"推荐提示词第 2/3 步中的 OpenAlex 引用及 429 处置说明。
  - 项目说明：`AGENTS.md`（项目概述的源清单与工具计数、数据源范围表中 OpenAlex 一行）。
  - 验证脚本：`_verify/tool_availability_check.py`（4 处 openalex 调用，含以 OpenAlex 搜索结果回填 DOI 的依赖）。
  - 未改动：`project-docs/teach.md`（用户此前明确"没必要更新"）；`goal.md`/`project-plan.md`/`buildlog.md` 中的历史条目按"不改写历史"惯例保持原样，本轮变更以追加方式记录。
- **目标状态**
  - 数据源 **10 → 9**；工具 **23 → 21**。注意与 QA-R018 的关键差异：OpenAlex 的 2 个工具是**无条件注册**的，因此本轮工具数**确实下降**（Semantic Scholar 那轮因无 Key 时本就注册 0 个工具，用户可见工具数未变）。
  - 全仓库（含以 `--no-ignore` 覆盖被 `.gitignore` 遮蔽的 `_verify/`）对 `openalex`/`OpenAlex` 的引用归零，仅保留历史文档条目。
  - 版本号维持 **`3.4.0`**：该版本号已由 QA-R018 一设定，但**尚未构建、尚未发布到 PyPI**，故本轮 OpenAlex 移除**并入同一未发布版本**，不另开 `3.5.0`（用户本轮指令亦为"更新版本到3.4.0"）。已发布的 v3.3.0 不受影响。
- **影响的目标文档章节**
  - 核心目标 / 范围界定（包含与排除） / 约束条件 / 成功标准 / 备注
...
- **提问目的**：用户要求"详细了解 CORE 官方 API 文档，还有什么功能可以加入到这个 MCP server"。本 agent 已完成**文档站 + OpenAPI 规范**双线调研，并用 `.env` 中真实 `CORE_API_KEY` 对文档列出的全部端点做了真实探测（端点清单、状态码、响应体大小、失败时的上游错误体详见上文《附录：CORE API v3 能力盘点（QA-R021 前置调研，2026-09-18）》）。调研结论是：CORE 目前只接入了 1 个工具（`core_work_search_by_query`，即 GET works 关键词检索），文档中另有约 18 项能力未接入——其中 **9 项实测可用**（works 详情（裸 DOI 可直接取详情）/ 生命周期 / 关联 outputs / 聚合统计、POST 高级检索含 `exclude` 字段裁剪、data-providers 检索与详情统计及其下 outputs、outputs 按 ID 取详情），**1 项实测存在但无价值**（journals 按 ISSN 查询——返回 200 但只有回显式空壳记录，建议不纳入），**5 项需复测或形态可疑**（journals 关键词检索连续超时、discover 500、recommend 500、TEI 返回 0 字节，以及 outputs 检索在 creator / root 两环境结果不一致），**3 项与项目既有立场直接冲突**（PDF/TEI/raw 文件下载）。因此在写入任何"核心目标/范围界定"条目之前，必须先由用户定三件事：扩展范围收到哪一档、"文件内容与下载"这条边界要不要为 CORE 破例、以及先复测再定范围还是直接按本次实测结论立项。
- **问题列表**
  1. **CORE 能力扩展的范围档位**——请选择一档（也可自定义组合）：
     (a) **只做现有工具的检索质量增强，工具数量保持 1 个**：`limit` 上限由 25 提到 100、补 `offset` 分页、改用 POST `exclude:["fullText"]` 把响应体压小、错误文案改为按 `X-RateLimit-*` 响应头给出可执行的重试时间。**该项有直接实测收益**：现有工具在 `max_results=25`（其上限）时响应体 **676,692 字节 / 4.8 秒**，其中被 `_normalize()` 丢弃的 `fullText` 占 536,574 字符；改用 POST + `exclude:["fullText"]` 后同样 25 条 → **112,549 字节 / 2.8 秒**（体积约 1/6、耗时约 6 折）。属于修既有缺陷；
     (b) **在 (a) 之上新增 3 个 works 维度工具**（CORE 工具 1 → 4）：**按 DOI 或 CORE ID 取详情**（`/v3/works/{identifier}`，实测裸 DOI 直接可用）、取该作品在各机构库的实例列表（`/works/{id}/outputs`，注意该子资源**只接受数字 CORE ID**，用 DOI 会 404）、取生命周期时间戳（`/works/{id}/stats`）；
     (c) **在 (b) 之上再新增"聚合统计 + 机构库维度"工具**（CORE 工具约 6–8）：按年/作者/机构/类型分布统计（`/v3/search/works/aggregate`）、数据源检索与详情统计及其下 outputs（`/v3/search/data-providers`、`/v3/data-providers/{id}` 系列）。（**期刊维度已剔除**：`/v3/journals/issn:{issn}` 虽返回 200，但只有回显式空壳记录、无刊名/出版商/OA 状态，建议不纳入。）
     (d) 其他组合，请具体列出保留/剔除哪些（例如只要聚合统计，不要机构库维度）。
     同时请一并表态：CORE 这些新维度（机构库画像、分布统计）不与现有 9 个源重叠，是否按"新查询维度值得扩"处理；还是延续 v3.2.0–v3.4.0 的"继续收窄工具规模、降低选择噪声"方向，本轮只做 (a)。
  ...
  3. **验证节奏与目标版本号**——本次需复测项（journals 关键词检索超时、discover / recommend 各 500、TEI 空响应，以及 outputs 检索在 creator / root 两环境结果不一致）按 QA-R013 流程应由 project-builder-cn 编写 `_verify/core_api_probe.py` 交用户在真实环境复测后再定去留：
...
- **后续进展与闸门解除（2026-09-19，/root 记录）**：
  - **复测已完成（第三个环境）**：/root 在本机跑通了本轮产出的 `_verify/core_api_probe.py`（commit `ad95420`，纯标准库、IP 与 key 自动脱敏、只读不落地），结果：① 分层连通性 DNS→TCP→TLS(1.3)→HTTPS 四层全通过；② **对照组 6 项全部 200**（works 检索、裸 DOI 取详情、works limit=25、POST `exclude["fullText"]`、data-provider 详情、output 详情），证明网络/鉴权正常；③ 复测项中**唯一会改变工具清单的 `GET /v3/search/outputs` 三种查询组合 R6/R7/R8 全部 200**（累计 7 次连续 200、跨 2 个环境），此前那两次 500 更像上游瞬时故障或与特定表达式相关；④ 另外四项复现且**均不影响已定 C 档范围**——`/v3/search/journals` 超时（累计 4 次）、`POST /v3/discover` 500（下游 `oadiscovery` 404）、`POST /v3/recommend` 500（下游 recommender 500）、`/v3/works/tei/{id}` 一个 ID 200 但 0 字节另一个 404。
  - **决策闸门已由用户解除**：用户于 2026-09-19 指示"key 既然有效就不用管，开始更新计划书"，即 (a) 档的"先复测再立项"闸门视为满足，`.env` 中 `CORE_API_KEY` 的到期注释不再作为阻塞项处理。据此**实现范围锁定为 C 档的 9 项**（1 个现有工具增强 + 8 个新增），其中**第 9 项 `core_output_search_by_query` 为条件纳入**：证据支持纳入（7 次连续 200），但保留击杀条件——若用户自行复测或实现期探测出现非 200，则该工具不纳入、其余 8 项不受影响。
  - **目标版本号默认 `3.5.0`**（当前 `pyproject.toml` 为 `3.4.0`；用户尚未逐字确认该版本号，按既有惯例由计划书标注"如另有指定只需替换两处字面值"）。
  - **本轮范围条目仍未写入《核心目标 / 期望成果 / 成功标准 / 范围界定》**：用户指示直接进入计划书更新，故这些章节的范围条目改由 `project-planner-cn` 在 `project-docs/project-plan.md` 的步骤 69+ 中承载；若后续需要回填 goal.md 的范围章节，按本条的锁定结论补写即可。
- **影响的目标文档章节**
  - 本轮**暂不改动**《核心目标 / 期望成果 / 成功标准 / 范围界定》——按第 3 问选择的 (a) 档"先复测再立项"，这些章节的范围条目待复测结果回来后由本 agent 补写；截至 2026-09-19 仍未补写（用户指示直接进入计划书更新，详见上一条"后续进展与闸门解除"）
...
- （v3.0.0，QA-R013，2026-08-05）dblp 经用户三轮不同网络环境实测（TLS 握手失败 → HTTP 500 → 完全成功并返回真实数据）后正式定案纳入 v3.0.0，已记录已知风险（可能因网络环境波动出现间歇性连接失败，非服务下线/代码错误），处理方式（如是否在错误信息中加提示文案）留待实现阶段决定。**至此 v3.0.0 立项之初规划的 13 个数据源/功能点范围已全部定案完毕**：12 个确认落地（OpenAlex、Crossref、Europe PMC、DOAJ、Zenodo、HAL、OpenAIRE、bioRxiv/medRxiv、ChEMBL、Semantic Scholar、CORE、dblp）+ 1 个明确排除（PMC）+ 0 个悬而未决，加上已单独确认的 arXiv 补 `doi` 字段，**没有遗留的待用户决策事项**。用户同时提出一条新增的通用流程约束（已记入"约束条件"）：今后 v3.0.0 剩余全部实现工作及以后，agent 在自身环境中遇到失败结果（尤其网络类失败）时，不得仅凭自身单次结果下结论，必须把验证脚本产出到 `_verify/` 目录交由用户独立验证。**正式移交 `project-planner-cn` 续写 `project-plan.md` 步骤 34+**，规划全部剩余数据源的具体实现（含 dblp、Semantic Scholar 的条件注册架构、CORE 的 key 使用等），并在涉及验证脚本的步骤中遵循上述新流程约束。
- （v3.1.0，QA-R014/QA-R015，2026-08-07）v3.0.0 已在 QA-R013 全部定案发布。本轮目标澄清源自用户重启此前作废的 QA-R009（v2.4.0 调研）遗留决策点——"PubMed 检索改为直连 NCBI Entrez API"，QA-R014（范围对应方案C/新能力是否一并纳入/NCBI_API_KEY 注册模式）与 QA-R015（命名与 source 字段变更/新工具组织方式/字段对等性要求/版本号）两轮问题均已获用户明确回答，文档中不再有"待定/待用户确认"的占位状态。**下一步建议调用 `project-planner-cn` 基于本文档最终版制定 v3.1.0 构建计划书**，交接要点：
  1. **本轮是重构+新增混合性质，范围已完全收敛，无需再做探索性调研**：① 重写 `pubmed_paper_search_by_query`（新文件 `pubmed.py`，移除 `paperscraper`/`pymed-paperscraper` 依赖，改为直接 `httpx` 调用 `esearch.fcgi`+`efetch.fcgi`）；② 新增 3 个独立工具（ESummary 批量元数据、ELink 相关文献、ELink PMC 全文/引用关联）；③ 新增 `NCBI_API_KEY` 可选环境变量，无条件注册模式；④ 源码文件/注册函数名/README 等文档引用同步改名，JSON 响应体 `source` 字段值从 `"paperscraper"` 改为 `"pubmed"`。
  2. **正式实现前建议先做一次真实 API 复测，而非直接照搬历史探测结论**：QA-R009（已作废，探测结论见 QA-R014 提问目的完整复述）已用真实请求确认 ESummary/ELink 三项候选端点均返回 200 且不需要 Key，但探测发生在数日之前，project-builder-cn 应按本项目"真实验证优先"惯例重新核实一次响应结构、字段命名、限速表现，不应仅凭历史记录直接编写实现代码；用户 `.env` 中已配置真实 `NCBI_API_KEY`，具备条件验证提速效果。
...
- （v3.4.0，QA-R018/QA-R019，2026-09-18）v3.4.0 的范围由两轮**直接指令**定案，均无需澄清问答：① QA-R018 移除 Semantic Scholar（机构无法取得 Key，匿名关键词检索确定性 429）；② QA-R019 移除 OpenAlex（匿名检索端点持续 429，用户选择直接移除而非申请其免审批 Key）。两轮性质均为**删除型、范围小而封闭**，不需要项目规划专家介入做分阶段设计，可由 project-builder-cn 直接执行并在 `buildlog.md` 追加记录。**目标状态：9 个数据源 / 21 个工具**，全部无条件注册（无任何配置相关的工具数变化），版本号 `3.4.0`（**尚未构建、尚未发布**，故 v3.4.0 的两轮删源并入同一未发布版本）。交接要点：
  1. **文档计数与源小节必须同步，不得遗留"文档写了但代码没有"**：`README.md`、`README_ZH.md` 的特性列表、数据源对比表、`### OpenAlex` 小节、工具可用性实测段落、以及"文献查找"推荐提示词中的源引用，`AGENTS.md` 的项目概述与数据源范围表。
  2. **`_verify/tool_availability_check.py` 是全量工具可用性回归的唯一入口**，删源后需同步移除其中的对应调用；该脚本被本地 `.gitignore` 遮蔽，搜索引用时须用 `rg --no-ignore`，否则会漏检。
  3. **实际工具数一律以 `list_tools()` 返回值为准**，不得用 `rg -c "@server.tool"` 直接计数（历史上有注释中的该字符串导致虚高的先例）。
  4. 版本号 `3.4.0` 虽由用户本轮指令明确给出，但仍未获其逐字确认删除型改动的版本语义；如否决只需替换 `pyproject.toml` 与 `src/uniarticles/__init__.py` 两处字面值。
  5. `project-docs/teach.md` 按用户指示**不更新**（当前已滞后三轮，属已知失真，非本轮缺陷）。
